...
var QuickHull4D = (function (scope) {
	...
	QuickHull4D.prototype.computeInitialHull = function(){
		...
		if(facet.isPointAbove(v[0])){
			...
			facet = new Facet(v[4],v[3],v[2],v[1])
		}
		this.facets.push(facet);

		facet = new Facet(v[4],v[3],v[2],v[0]);
		if(facet.isPointAbove(v[1]))
			facet = new Facet(v[0],v[2],v[3],v[4]);
		this.facets.push(facet);

		facet = new Facet(v[0],v[1],v[3],v[4]);
		if(facet.isPointAbove(v[2]))
			facet = new Facet(v[4],v[3],v[1],v[0]);
		this.facets.push(facet);

		facet = new Facet(v[4],v[2],v[1],v[0]);
		if(facet.isPointAbove(v[3]))
			facet = new Facet(v[0],v[1],v[2],v[4]);
		this.facets.push(facet);

		facet = new Facet(v[0],v[1],v[2],v[3]);
		if(facet.isPointAbove(v[4]))
			facet = new Facet(v[3],v[2],v[1],v[0]);
		this.facets.push(facet);
	}

	QuickHull4D.prototype.addVertexToHull = function(v){
		/* 
			Adds a single new vertex v to the hull
		*/
	...
	QuickHull4D.prototype.computeHorizon = function(eyePoint){
		/* 
			...
			// 9- Update assignments
				/* 
					For each new facet F' 
						for each unassignment point q in an outside set of a facet in V 
							if q is above F'
	...
	Object.assign( Facet.prototype, {
		getHyperPlane: function(){
		    ...
		    normal = normal.cross(v1, v2, v3);
		    var p0 = this.vertices[0];
		    var plane = new HyperPlane(normal,p0);
		    return plane;
		},
