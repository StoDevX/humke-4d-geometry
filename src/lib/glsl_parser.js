/*!
 Based on the parser library, modified to spit out GLSL code. 
*/
...
var GLSLParser = (function (scope) {
	...
	Expression.prototype = {
		...
		substitute: function (variable, expr) {
			if (!(expr instanceof Expression)) {
				expr = new GLSLParser().parse(String(expr));
			}
			var newexpression = [];
			var L = this.tokens.length;
			var item;
		...
		toString: function (toJS) {
			...
			for (i = 0; i < L; i++) {
				...
				var type_ = item.type_;
				if (type_ === TNUMBER) {
					// Must convert all numbers to floats! 
					var num = escapeValue(item.number_).toFixed(1);
					nstack.push(num);
				}
				else if (type_ === TOP2) {
					n2 = nstack.pop();
	...
	GLSLParser.parse = function (expr) {
		return new GLSLParser().parse(expr);
	};

	GLSLParser.evaluate = function (expr, variables) {
		return GLSLParser.parse(expr).evaluate(variables);
	};

	GLSLParser.Expression = Expression;
